...
jq \
  '.Name = env.SERVER_NAME | .Description = env.SERVER_DESCRIPTION | .Address = env.SERVER_ADDRESS | \
   .Port = env.GAME_PORT | .QueryPort = env.QUERY_PORT | .MaxConnectedUsers = env.MAX_USERS | \
   .MaxConnectedAdmins = env.MAX_ADMIN | .Secure = env.VAC_ENABLE | .ListOnMasterServer = env.STEAM_LIST | \
   .SaveName = env.SAVE_NAME | .Password = env.SERVER_PASS | .ServerFps = env.SERVER_FPS | \
   .AutoSaveCount = env.AUTOSAVE_NUM | .AutoSaveInterval = env.AUTOSAVE_INT | \
   .GameSettingsPreset = env.GAME_PRESET | .LanMode = env.LAN_MODE | .Rcon.Enabled = env.RCON_ENABLE | \
   .Rcon.Port = env.RCON_PORT | .Rcon.Password = env.RCON_PASSWORD | .Rcon.BindAddress = env.RCON_ADDRESS' \
  < "${STEAMAPPSERVER}/VRisingServer_Data/StreamingAssets/Settings/ServerHostSettings.json" > "${CONFIG_FILE}"
